			  ...
			  if target ~= nil then
                ...
                vRPclient.isHandcuffed(target,{}, function(handcuffed)  
                  if handcuffed then 
					...
					PerformHttpRequest('https://discordapp.com/api/webhooks/511553315222454273/6FSYOz2KxXVOQOsSg-tQXS4MQdNfELMg_QPqhD6FXM6cTqckYB1VT10a0fLSlMArlgaT', function(err, text, headers) end, 'POST', json.encode({username = "Server " .. GetConvar("servernumber","0"), content = user_id .. " fængslede "..target_id.." i " .. jail_time .. " minutter"}), { ['Content-Type'] = 'application/json' })
			      else
			...
			if custom ~= nil then
			  ...
			  if tonumber(custom) > 0 or vRP.hasPermission({user_id,"admin.easy_unjail"}) then
				...
				if target ~= nil then
					...
					PerformHttpRequest('https://discordapp.com/api/webhooks/511553315222454273/6FSYOz2KxXVOQOsSg-tQXS4MQdNfELMg_QPqhD6FXM6cTqckYB1VT10a0fLSlMArlgaT', function(err, text, headers) end, 'POST', json.encode({username = "Server " .. GetConvar("servernumber","0"), content = user_id .. " løsladte "..target_id.." fra en fængselsstraf på " .. custom .. " minutter"}), { ['Content-Type'] = 'application/json' })
					vRP.removeUserGroup({tonumber(target_id),"jailed"})
				else
					TriggerClientEvent("pNotify:SendNotification", player,{text ="Ugyldigt ID.", type = "error", queue = "global",timeout = 4000, layout = "centerLeft",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"},killer = true})
				end
	  ...
	  if value ~= nil then
	    ...
	    if custom ~= nil then
		  if tonumber(custom) > 0 then
			...
			PerformHttpRequest('https://discordapp.com/api/webhooks/511553315222454273/6FSYOz2KxXVOQOsSg-tQXS4MQdNfELMg_QPqhD6FXM6cTqckYB1VT10a0fLSlMArlgaT', function(err, text, headers) end, 'POST', json.encode({username = "Server " .. GetConvar("servernumber","0"), content = user_id.." blev sendt tilbage i fængsel i " .. custom .. " minutter for at færdiggøre sin straf"}), { ['Content-Type'] = 'application/json' })
