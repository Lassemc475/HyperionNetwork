    ...
    window.addEventListener('message', function(event){
        ...
        if(item.mode == "profil") {
            ...
            if(json.length == undefined) {
                ...
                $("#profil div").append("<ul class='info'><li>Køretøj:</li><li>Pris i DKK:</li><li>Status:</li><li></li></ul>")
                for(var i = 0; i != json.length; i++) {
                    var status = "Afventer";
        ...
        if(item.mode == "search") {
            ...
            if(json.length == undefined) {
                if(search == "") {
                    $("#search-result").text("Der blevet ikke fundet nogle resultere!");
                }else{
                    $("#search-result").text("Der blevet ikke fundet nogle resultere under: "+search+"!");
                }
            }else {
                for (var i = 0; i != json.length; i++) {
