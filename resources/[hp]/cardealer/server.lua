...
MySQL.createCommand("vRP/cardealer_getAI","SELECT AUTO_INCREMENT FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'vrpfx' AND TABLE_NAME = 'vrp_cardealer'")
MySQL.createCommand("vRP/cardealer_search","SELECT * FROM vrp_cardealer WHERE cardealer_deleted = '0' AND cardealer_accepted ='1' AND cardealer_vehiclename LIKE @search ORDER BY cardealer_created ASC")
                        ...
                        if #row2 > 0 then
                            ...
                            if (vRP.tryFullPayment({user_id,price})) then
                                if isselleronline then
                                    ...
                                    if pseller ~= nil then
                                        if vRP.tryDeposit({seller,price}) == false then
                                            local sbank = vRP.getBankMoney({seller}) + price
                                            vRP.setBankMoney({seller,sbank})
                                        end
                                        TriggerClientEvent("pNotify:SendNotification", pseller, {text = "Din <b style='color:lime'>"..vehiclename.."</b> er lige blevet solgt!", type = "success", timeout = 4000, layout = "centerLeft"})
                                ...
                                MySQL.query("vRP/get_user_identity", {user_id = user_id}, function(row, affected)
                                    registration = row[1]["registration"]
                                end)
                                Citizen.Wait(100)
                                registration = "P"..registration
                                MySQL.query("vRP/cardealer_updatePlate", {plate = registration, user_id = seller, vehicle = vehicle})
                                Citizen.Wait(100)
                                TriggerClientEvent("pNotify:SendNotification", lsource, {text = "Tillykke med din nye <b style='color:lime'>"..vehiclename.."</b>!", type = "success", timeout = 4000, layout = "centerLeft"})
...
AddEventHandler('cardealer:create', function(vehname, veh, vehdesc, price, img)
    ...
    Citizen.CreateThread(function()
        ...
        local id = name.." - ("..tonumber(user_id)..")"
        local link = "http://second.terstedmikkelsen.dk/cms/cardealer.php?mode=details&id="..ai
        TriggerClientEvent("cardealer:discord", lsource, "https://discordapp.com/api/webhooks/495595764999913473/ANRpR_18L9_jwwdfiRakqHo5geRJJF2xwRFrcoRjd-MVZBUpcBGQI_Bzm_qjXYS4Sd73" , id, img, vehname, veh, price, vehdesc, link)
    end)
end)